...
const focusMusicData: MusicItem[] = [
  ...
  },
  {
    ...
    url: "https://www.youtube.com/watch?v=YJKDpKu9-nU",
    duration: "4:00:00", 
    genre: "Classical",
...
export default function MusicPage() {
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as "focus" | "break" | "energize")}>
        ...
        <TabsContent value="break" className="space-y-4 mt-6">
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {filteredMusic.map((item) => (
              <MusicCard
                key={item.id}
        ...
        <TabsContent value="energize" className="space-y-4 mt-6">
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {filteredMusic.map((item) => (
              <MusicCard
                key={item.id}
...
function MusicCard({ item, onPlay, isLoading }: MusicCardProps) {
  ...
  return (
    <Card className="hover:shadow-lg transition-all duration-200 hover:border-tracksy-gold/30">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex gap-2">
          <Button
            onClick={onPlay}
            disabled={isLoading}
            className="flex-1 bg-tracksy-gold hover:bg-tracksy-gold/90 text-white"
            size="sm"
          >
